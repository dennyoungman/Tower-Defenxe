...
const camera={x:WORLD.w*.65,y:WORLD.h*.66,zoom:1,shake:0};
const player={x:camera.x,y:camera.y,a:-Math.PI/2,v:0,r:15,cash:1250,heat:0,wanted:0};
const missionDefs=[
...
function parseOSM(elements){
  ...
  for(let i=0;i<Math.min(26,eligible.length*2);i++){
    ...
    traffic.push({route,dist:(route.len*((i*0.173)%1)),dir,speed:48+(i%6)*7,lane:(i%2?1:-1)*Math.min(15,route.width*.2),color:carColors[i%carColors.length]});
  }
...
function updatePlayer(dt){
  ...
  const steps=Math.max(1,Math.ceil(Math.abs(player.v)*dt/8));
...
function updateTraffic(dt){
  if(!ready)return;
  for(const c of traffic){c.dist+=c.speed*c.dir*dt;if(c.dist>c.route.len){c.dist=c.route.len;c.dir=-1;}if(c.dist<0){c.dist=0;c.dir=1;}const p=trafficPose(c);c.x=p.x;c.y=p.y;c.a=p.a;const dx=player.x-c.x,dy=player.y-c.y,d=Math.hypot(dx,dy);if(d<37){const nx=dx/(d||1),ny=dy/(d||1),push=37-d;player.x+=nx*push*.75;player.y+=ny*push*.75;player.v*=-.08;c.dist-=c.dir*push*.2;camera.shake=4;}}
}
function updateMission(){if(!ready||!missions.length)return;const m=missions[missionIndex];if(Math.hypot(player.x-m.x,player.y-m.y)<48){player.cash+=500;showToast('JOB COMPLETE  +€500');missionIndex=(missionIndex+1)%missions.length;const n=missions[missionIndex];ui.objective.innerHTML='<b>JOB</b><span>Drive to '+n.name+'.</span>';}}
function update(dt){updatePlayer(dt);updateTraffic(dt);updateMission();camera.x+=(player.x-camera.x)*Math.min(1,dt*5);camera.y+=(player.y-camera.y)*Math.min(1,dt*5);camera.zoom+=(1+Math.min(.12,Math.abs(player.v)/2200)-camera.zoom)*dt*3;camera.shake=Math.max(0,camera.shake-dt*18);ui.cash.textContent='€'+player.cash.toLocaleString();ui.speed.textContent=Math.round(Math.abs(player.v)*.72);ui.wanted.textContent='☆☆☆☆☆';if(toastTimer>0){toastTimer-=dt;if(toastTimer<=0)ui.toast.classList.remove('show');}}
...
function drawCar(c,playerCar=false){ctx.save();ctx.translate(c.x,c.y);ctx.rotate(c.a+Math.PI/2);ctx.fillStyle='#0006';rr(-12+4,-25+5,24,50,6);ctx.fill();ctx.fillStyle=playerCar?'#d9473f':c.color;rr(-12,-25,24,50,6);ctx.fill();ctx.fillStyle='#9bc9d7';ctx.fillRect(-8,-12,16,12);ctx.fillStyle='#19343b';ctx.fillRect(-7,-10,14,9);ctx.fillStyle='#9bc9d7';ctx.fillRect(-8,5,16,10);ctx.fillStyle='#f5da84';ctx.fillRect(-9,-24,5,3);ctx.fillRect(4,-24,5,3);ctx.restore();}
function drawMission(){if(!ready||!missions.length)return;const m=missions[missionIndex],pulse=10+Math.sin(performance.now()/180)*3;ctx.strokeStyle='#ffd94a';ctx.lineWidth=4;ctx.beginPath();ctx.arc(m.x,m.y,30+pulse,0,Math.PI*2);ctx.stroke();ctx.fillStyle='#ffe26a';ctx.font='700 16px system-ui';ctx.textAlign='center';ctx.fillText(m.name,m.x,m.y-48);}
function drawLabels(){ctx.save();ctx.font='700 13px system-ui';ctx.textAlign='center';ctx.fillStyle='rgba(225,231,232,.48)';let count=0;for(const r of roadLabels){if(count++>18)break;const p=sampleRoute(r,r.len*.5);ctx.save();ctx.translate(p.x,p.y);ctx.rotate(p.a);ctx.fillText(r.name,0,-r.width*.6-5);ctx.restore();}ctx.restore();}
